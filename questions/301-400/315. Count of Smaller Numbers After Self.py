...
class Solution:
    def countSmaller(self, nums: List[int]) -> List[int]:
        ...
        for i in range(n - 1, -1, -1):
            idx = binary_search(sorted_nums, nums[i])
            res.append(idx)
            sorted_nums.insert(idx, nums[i])
        return res[::-1]

# generate another solution to iterate from the end of the array
# using the bisect_left to find the index of the element in the sorted array.
